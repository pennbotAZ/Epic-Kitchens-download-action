...
class EpicDownloader:
    ...
    def download_items(self, video_dicts, epic_55_parts_func, epic_100_parts_func, from_url=None, output_parts=None):
        with open("action.csv", 'r') as csvfile:
            ...
            for action_list in r:
                print(action_list)

        for a in action_list:
            action_path = os.path.join(self.base_output, a)
            if not os.path.exists(action_path):
                os.mkdir(action_path)

        action_dicts = {}
        with open("data/EPIC_100_train.csv", 'r') as csvfile:
            reader = csv.DictReader(csvfile, delimiter=',')
            for row in reader:
                if row['narration'] in action_list:
                    # frame_xxxxxxxxxx.jpg
                    if row["video_id"] not in action_dicts:
                        action_dicts[row["video_id"]] = [[row['narration'], int(row["start_frame"]), int(row["stop_frame"])]]
                    else:
                        action_dicts[row["video_id"]].append([row['narration'], int(row["start_frame"]), int(row["stop_frame"])])
        ...
        for video_id, d in video_dicts.items():
            ...
            if self.file_already_downloaded(output_path, remote_parts, version):
                ...
                if video_id in action_dicts:
                    self.download_file(url, output_path)
                    print(output_path)
                        
                    mytar = tarfile.open(output_path)
                    ext_path = output_path.rstrip(".tar")
                    mytar.extractall(ext_path)
                    os.remove(output_path)
                    for clip in action_dicts[video_id]:
                        action_path = os.path.join(self.base_output, clip[0]) + "/" + str(video_id) 
                        if not os.path.exists(action_path):
                            os.mkdir(action_path)
                        for i in range(clip[1], clip[2]):
                            frame = 'frame_{0:010}.jpg'.format(i)
                            shutil.copyfile(os.path.join(ext_path, frame), os.path.join(action_path, frame))
...
def create_parser():
    ...
    parser.add_argument('--action-path', nargs='?', type=str, default="action.csv")
